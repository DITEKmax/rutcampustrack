...
var HeadmanApiService = class _HeadmanApiService {
   ...
   */
  getPendingLateCheckins() {
    return this.http.get("/api/attendance/late-checkin/pending").pipe(map((resp) => {
      if (Array.isArray(resp))
        return resp;
      const embedded = resp?._embedded;
      if (!embedded)
        return [];
      return embedded["lateCheckinRequestResponseList"] ?? Object.values(embedded)[0] ?? [];
    }), catchError((err) => {
   ...
   */
  decideLateCheckin(requestId, approved) {
    return this.http.post(`/api/attendance/late-checkin/${requestId}/decision`, { approved });
  }
  /**
...
//# sourceMappingURL=chunk-BU5FJGI6.js.map
