...
- **Корневой фикс.**
  ...
  4. Добавить в `docs/architecture.md` раздел «Секреты и их ротация» — какой секрет где используется, как ротировать, кто владеет.
...
5. **Ротация секретов — раз в квартал** (C0-9): chron-задача в команде, `docs/architecture.md` → список + последняя ротация.
