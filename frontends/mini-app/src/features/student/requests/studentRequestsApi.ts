...
async function fetchStudentAttendanceRecords(): Promise<AttendanceRecord[]> {
  ...
  return unwrapEmbeddedList<AttendanceRecord>(data, ['attendanceRecordEntryList']).map((record) => ({
    ...record,
...
function normalizeTicket(ticket: ExcuseTicket): ExcuseTicket {
  return {
    ...ticket,
    status: ticket.status.toLowerCase() as ExcuseTicket['status'],
...
export function useStudentAttendanceRecords(enabled = true) {
