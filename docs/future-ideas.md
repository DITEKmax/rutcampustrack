...
## Auth API contract-first refactor (v0.1, отложен в M09 G8)

**Текущее состояние (v0.0.0):** `auth-service` — единственный сервис
v0.0.0, нарушающий правило «contract-first» (см. CLAUDE.md). Контроллер
`AuthController` напрямую объявляет REST-endpoint'ы (`@PostMapping`
в самом классе), без выделения `auth-api-contract` Gradle-модуля.
Frontend-типы генерируются через `generate:types:offline` из
`docs/openapi/auth.json` ручной конкатенации — это не single source of
truth.

Owner-решение 01-Q1: 01 P0-1 **отложен в v0.1 backlog**. Обоснование:
- Auth-service стабилизирован 4 milestone'а назад (v1.0), рефакторинг
  не несёт функционал.
- Rollback cost существенный — frontend tests завязаны на текущий
  shape DTO (даже сгенерённых).
- M09 — prod-release-blockers only; non-functional refactoring не
  должен расширять scope.

**v0.1 план:**
1. Создать `services/auth-service/auth-api-contract` как
   `java-library` (БЕЗ Lombok) с:
   - `AuthApi` interface (все 6 endpoint'ов через `@Operation`/`@PostMapping`)
   - `WsTicketApi` interface (1 endpoint)
   - `InternalAuthApi` interface (token exchange)
2. Мигрировать DTO:
   - Request DTO → Java `record` (OtpRequest, TokenRefreshRequest...)
   - Response DTO → class (TokenResponse, OtpVerifyResponse...)
3. `auth-app` зависит от `auth-api-contract`, контроллер
   `implements AuthApi`.
4. Регенерировать `docs/openapi/auth.json` через Springdoc на лету
   (sharedopenapi customizer).
5. Добавить **CI-правило** в ArchUnit/OpenAPI-drift: auth-service
   больше не исключение.

**Оценка:** ~4-5 человеко-дней (включая тестовое покрытие + PR cycles
с owner).

**Зависимости:** M12 в v0.0.0 roadmap (Auth Contract-first Refactor) —
планирование; фактическая реализация — v0.1.

---

## Auth-service OpenAPI (P2-2/2, v0.1)

**Текущее состояние (v0.0.0):** `docs/openapi/auth.json` — **вручную**
конкатенированный файл, не отражает реальных DTO-аннотаций (т.к.
CLAUDE.md правило `@Schema` на interface недоступно — auth нарушает
contract-first, см. выше).

**Связано с** Auth API contract-first refactor (v0.1). После миграции
auth на `auth-api-contract` module:
1. Springdoc scan'ит `AuthApi` interface + DTO records.
2. `SharedOpenApiCustomizer` (внутрисервисный) сливает info/servers.
3. `openapi-drift.yml` workflow сравнивает generated vs commited
   `auth.json` — делает это автоматическим (как для academic/schedule/
   attendance сейчас).
4. Удаляем ручную конкатенацию из release procedure.

**Оценка:** +1 день поверх contract-first refactor (мало новой работы,
