...
- **M16**: ЗАВЕРШЕН (Post-Deploy Hardening) — 9 групп G1-G9, 9 коммитов на main `fe652149..f435e98f` + docs `dafb38d2`, начало и финал 2026-04-27. Verify на VPS отложен до redeploy `v0.0.0-alpha.17`
...
| M15 | First VPS Deploy | ✅ Деплой на VPS 2026-04-27. 4 hotfix-коммита: `b8cf106` suppress 3 transitive CVEs + 4 frontend Dockerfile DS-0002 + `c7b2b93` cosign verify case-insensitive + `c3ff148` env regression (INTERNAL_ISSUER_SECRET) + OOM (notification-web mem_limit) + alertmanager time_intervals split + `8d7c168` nginx CSP allow Angular inline onload (inlineCritical CSS lazy-load). Retrospective papka — `docs/milestones/M15-first-vps-deploy/` (PLAN+NOTES, без CHECKLIST/DECISIONS — milestone закрыт по факту в проде) |
| M16 | Post-Deploy Hardening | ✅ 9 групп закрыты 2026-04-27. G1 OTel exporter port (4317→4318, mixed-mode Java HTTP/Python gRPC) + G2 dispatcher exception propagation + DLQ retention 7d (idempotency была M13 G8) + G3 OTP brute-force counter `verifyOtpByCode` 20/5min/IP + alert OtpBruteForceSuspect + G4 Loki startup race (`min_ready_duration: 15s` + healthcheck-gated promtail) + G5 nginx DNS race (resolver 127.0.0.11 + 8 upstream variables) + verify-deploy 4 false-positives + G6 @AdminAction real audit log v1 (V19+V20 + SPI + JdbcAuditLogStorage + 5 UserController actions, без diff/auto-target) + G7 headman RL Redis 300/min (interface + Redis primary + InMemory fallback, fail-open) + **G8a `cap_drop: [NET_RAW]` для 4 infra-контейнеров** (cadvisor, node-exporter, blackbox-exporter, alertmanager) — defense-in-depth против sniff'а в private_net + **G9 cadvisor de-privileged** (`devices: /dev/kmsg` + default caps, без SYS_PTRACE per cadvisor #3051). **G8b full mTLS отложен** в `future-ideas.md` MED-11b с trigger conditions (multi-host / compliance / first incident, см. DECISIONS § D13). Verify на VPS отложен до redeploy `v0.0.0-alpha.17`. |
...
M15 — retrospective short (PLAN+NOTES); M16 — полная структура.
