  ...
  const absentRecords = useMemo(
    () =>
      (recordsQuery.data ?? []).filter(
        (record) => normalizedStatus(record) === 'absent' && isPastOrToday(record.lessonDate),
      ),
    [recordsQuery.data],
  )
              ...
              >
                {group.records.map((record) => (
                  <LateCheckinRow
                    ...
                    onRequest={() => {
                      void requestForRecord(record.lessonId)
                    }}
                  />
                ))}
              </div>
  ...
  return (
    <header className="mb-5 flex items-start gap-3">
      ...
      <div className="min-w-0">
        ...
        <h1 className="text-2xl font-bold leading-tight text-balance">Запрос отметки</h1>
      </div>
    </header>
  )
}
...
function LateCheckinRow({
  ...
  return (
    <article className="border-b px-4 py-3 last:border-b-0" style={{ borderColor: 'var(--border-subtle)' }}>
