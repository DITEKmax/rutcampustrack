...
|-------|------|-------------|--------|
| 1 | Auth Service Core (JWT + Login) | - | completed |
| 2 | Auth Service OTP + Change Password | 1 | planned |
| 3 | API Gateway JWT Filter + Routing | 1 | planned |
| 4 | Seed Data + Integration Testing | 1, 2, 3 | pending |

> **Note (2026-04-27 normalization):** phases were originally numbered 1.1/1.2/1.3/1.4 as sub-phases of a single Phase 1. They were renormalized to 1/2/3/4 to align with the project-wide sequential numbering (v2.0 starts at Phase 5). File names inside the phase directories follow the standard `{NN}-RESEARCH.md`, `{NN}-{MM}-PLAN.md`, `{NN}-{MM}-SUMMARY.md`, `{NN}-VERIFICATION.md` pattern.
...
## Phase 2: Auth Service OTP + Change Password
...
Plans:
- [x] 02-01-PLAN.md — OTP flow (request/verify) with Redis rate limiting + change password endpoint
...
## Phase 3: API Gateway JWT Filter + Routing
...
Plans:
- [ ] 03-01-PLAN.md — JWT authentication filter (PublicKeyConfig + JwtAuthenticationFilter GlobalFilter) + routing verification
...
## Phase 4: Seed Data + Integration Testing
...
Plans:
- [x] 04-01-PLAN.md — Testcontainers integration tests for all auth endpoints (login, refresh, logout, OTP, change-password)
