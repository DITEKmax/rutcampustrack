...
- **Что:** Коллекция `push_subscriptions` живёт в БД `attendance_db`, которой владеет Attendance Service. Это прямое нарушение принципа **Database per Service** из `docs/architecture/architecture.md` §1 и §6. В архитектурном документе написано, что Notification Web имеет свою MongoDB `push_subscriptions` (см. матрицу «сервис → хранилище», строка Notification Web, столбец MongoDB `✅ (push_subscriptions)`).
