...
| M16 | Post-Deploy Hardening | ⏳ В РАБОТЕ — 9 групп: G1 OTel exporter port (4317→4318) + G2 event dispatcher idempotency (Redis SET NX) + G3 OTP brute-force counter (HIGH SA-H1) + G4 Loki InstancesCount diagnose + G5 nginx DNS race (resolver 127.0.0.11) + G6 @AdminAction audit log (real handler) + G7 headman rate-limit Redis 300/min + G8 mTLS Alertmanager → notification-web + G9 cadvisor de-privileged. **Magic-link и 🟢 мелочь отложены** в `future-ideas.md`. Старт 2026-04-27 |
