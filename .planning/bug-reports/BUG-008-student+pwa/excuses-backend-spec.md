...
1. STUDENT может создать тикет **только за прошедшие или текущие** уроки своей группы (lessonIds должны принадлежать `user.groupId`).
...
3. Староста **не может** одобрять/отклонять собственный тикет.
