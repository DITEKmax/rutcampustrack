...
function normalizedStatus(record: AttendanceRecord): string {
...
export function StudentExcusesPage() {
  ...
  const subjectIds = useMemo(() => records.map((record) => record.subjectId), [records])
    ...
    () =>
      records
        .filter((record) => {
          ...
          return (status === 'absent' || status === 'excused') && isPastOrToday(record.lessonDate)
        })
        .sort(
          (a, b) =>
            a.lessonDate.localeCompare(b.lessonDate) || a.lessonNumber - b.lessonNumber,
        ),
    [records],
  )
        ...
        >
          {visibleTickets.map((ticket) => (
            <TicketRow
              ...
              lessonLabel={(lessonId) => lessonLabel(lessonId, recordByLessonId, subjectMap)}
            />
          ))}
        </div>
      )}
    ...
    <header className="mb-5 flex items-start justify-between gap-3">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <h1 className="text-2xl font-bold leading-tight text-balance">Уважительные пропуски</h1>
        </div>
      </div>
...
function ExcuseFormSheet({
  ...
  }, [comment, excuseType, file, onClose, onSubmitted, selectedIds, submit])
  ...
  useMainButton({
    text: submit.isPending ? 'Отправляем...' : 'Отправить тикет',
    isEnabled: open && !submit.isPending,
    isVisible: open,
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          color: status === 'absent' ? 'var(--accent-danger)' : 'var(--accent-warning)',
          borderColor:
            status === 'absent'
